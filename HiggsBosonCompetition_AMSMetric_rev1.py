...
def AMS_metric(solution, submission, nEvents):
    ...
    numEvents = nEvents#125000#550000 # number of events = size of test set
    ...
    if check_submission(submission, numEvents):
        ...
        return str(AMS(signal,background))
    else:
        return "0.0"
...
if __name__ == "__main__":
    ...
    path = "/media/swap/Htautau/"
    solutionFile = "solutions.csv"
    submissionFiles = ['dtc.out','ada.out','bdt_real.out','gdb.out','clsnn.out','lognn.out','lognntx.out']
    for submissionFile in submissionFiles:
        print submissionFile
        nEvents = mapcount(submissionFile)-1
        print AMS_metric(solutionFile, submissionFile, nEvents)
